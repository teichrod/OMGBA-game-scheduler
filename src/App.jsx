...
function fairnessScore(team) {
  ...
  return (
    (team.earlyGames || 0) * 2 +
    (team.maxSameTimeSlot || 0) * 10 +
    Math.abs((team.home || 0) - (team.away || 0)) * 1.5 +
    (team.doubleHeaders || 0) * 2 +
    dayPartImbalance * 12 +
    Math.abs(morningGames - idealMorning) * 8 +
    Math.abs(afternoonGames - idealAfternoon) * 8
  );
}
...
function canPairInSlot(teamA, teamB, slot, config) {
  ...
  if (config.fifthBoysDoubleheaderDate) {
    if (teamA.division === "5th Boys" && slot.date !== config.fifthBoysDoubleheaderDate) {
      const aDhCount = teamA.gamesByDate[config.fifthBoysDoubleheaderDate] || 0;
      const bDhCount = teamB.gamesByDate[config.fifthBoysDoubleheaderDate] || 0;
      if (aDhCount < 2 || bDhCount < 2) return false;
    }
...
function getProjectedDayPartPenalty(team, slotTime) {
  ...
  const idealAfternoon = getIdealAfternoonGames(team);
  return (
    Math.abs(projectedMorning - projectedAfternoon) * 14 +
    Math.abs(projectedMorning - idealMorning) * 12 +
    Math.abs(projectedAfternoon - idealAfternoon) * 12
  );
}
...
function slotPenalty(teamA, teamB, slot) {
  ...
  penalty += getProjectedTimeCount(teamA, slot.time) * 18;
...
function scheduleGame(schedule, slot, teamA, teamB) {
  ...
  schedule.push({ division: homeTeam.division, date: slot.date, time: slot.time, court: slot.court, home: homeTeam.name, away: awayTeam.name });
}
...
function scheduleFifthBoysDoubleheaderDay(teams, openSlots, schedule, unscheduled, config) {
  ...
  if (teamList.length % 2 === 1) {
    unscheduled.push({ matchup: "5th Boys doubleheader day", reason: "5th Boys team count is odd", suggestion: "Use an even number of 5th Boys teams for an everyone-plays doubleheader day" });
    return;
  }
  ...
  if (slots.length < neededGames) {
    unscheduled.push({ matchup: "5th Boys doubleheader day", reason: "Not enough slots on selected date", suggestion: "Enable more courts or times on that Saturday" });
    return;
  }
  const selectedPairings = [...(rounds[0] || []), ...(rounds[1] || [])];
  if (selectedPairings.length < neededGames) {
    unscheduled.push({ matchup: "5th Boys doubleheader day", reason: "Could not build two full rounds of pairings", suggestion: "Adjust 5th Boys team count" });
    return;
  }
...
function generateScheduleEngine(config) {
  ...
  for (const division of DIVISIONS) {
    ...
    while (divisionTeams.some((t) => t.gamesScheduled < t.targetGames) && safety < 8000) {
      safety += 1;
      const needyTeams = divisionTeams.filter((t) => t.gamesScheduled < t.targetGames)
      ...
      for (const team of needyTeams) {
        ...
        const best = chooseBestCandidate(team, teams, freeSlots, config);
        if (!best) continue;
      ...
      if (!scheduledOne) {
        unscheduled.push({ matchup: needyTeams.map((t) => t.name).join(", "), reason: "No valid matchup or slot found during main pass", suggestion: "Add Saturdays, add courts on specific dates, or relax constraints" });
        break;
      }
    }
  }
  const remainingTeams = teams.filter((t) => t.gamesScheduled < t.targetGames)
    .sort((a, b) => (b.targetGames - b.gamesScheduled) - (a.targetGames - a.gamesScheduled));
  for (const team of remainingTeams) {
    let safety = 0;
    while (team.gamesScheduled < team.targetGames && safety < 500) {
      safety += 1;
      const slot = openSlots.find((s) => !s.used);
      if (!slot) {
        unscheduled.push({ matchup: team.name, reason: "No slots left", suggestion: "Add more Saturdays or enable more courts on active dates" });
        break;
      }
      ...
      if (!opponent) {
        unscheduled.push({ matchup: team.name, reason: "No opponent found", suggestion: "Allow more doubleheaders or create more slot capacity" });
        break;
      }
  ...
  const fifthBoysDhTeamsMet = teams.filter((t) => t.division === "5th Boys").every((t) => {
    if (!config.fifthBoysDoubleheaderDate) return true;
  ...
  const auditSummary = {
    ...
    earlyViolations: auditRows.filter((r) => r.early > Number(config.maxEarlyGames)).length,
    homeAwayIssues: auditRows.filter((r) => Math.abs(r.home - r.away) > 2).length,
    missingTeams: auditRows.filter((r) => r.games !== r.target).length,
    enabledDates: config.saturdays.filter((d) => d.enabled).length,
    enabledCourts: Object.values(config.dateCourtSettings).reduce((sum, courts) => sum + courts.filter((c) => c.enabled && String(c.name || "").trim() !== "").length, 0),
    fifthBoysDhTeamsMet,
  };
...
export default function App() {
  ...
  function toggleSaturday(index, enabled) {
    setConfig((prev) => ({ ...prev, saturdays: prev.saturdays.map((entry, i) => i === index ? { ...entry, enabled: Boolean(enabled) } : entry) }));
  }
  ...
  function toggleTime(index, enabled) {
    setConfig((prev) => ({ ...prev, timeSlots: prev.timeSlots.map((entry, i) => i === index ? { ...entry, enabled: Boolean(enabled) } : entry) }));
  }
  ...
  function runScheduler() {
    setResult(generateScheduleEngine(config));
    setScheduleDivisionFilter("all");
    setScheduleTeamFilter("all");
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
            ...
            {!isPublicMode ? (
              <>
                ...
                <button style={styles.primaryButton} onClick={runScheduler}>
                  <span style={{ display: "inline-flex", alignItems: "center", gap: 8 }}><Wand2 size={16} /> Generate Schedule</span>
                </button>
              </>
            ) : null}
          </div>
        ...
        {isPublicMode ? (
          <div style={styles.publicBanner}>Public view: schedule-only mode. Use filters below to browse by division or go straight to a specific team schedule.</div>
        ) : null}
                ...
                <div style={{ display: "grid", gap: 16 }}>
                  <div>
                    ...
                    <select style={styles.select} value={String(config.seasonYear)} onChange={(e) => changeSeasonYear(e.target.value)}>
                      {SEASON_YEAR_OPTIONS.map((year) => <option key={year} value={String(year)}>{year}-{String(year + 1).slice(-2)}</option>)}
                    </select>
                  </div>
                  <div>
                    ...
                    <select style={styles.select} value={config.fifthBoysDoubleheaderDate || "none"} onChange={(e) => setConfig((prev) => ({ ...prev, fifthBoysDoubleheaderDate: e.target.value === "none" ? "" : e.target.value }))}>
                      ...
                      {decemberSaturdayOptions.map((entry) => <option key={entry.date} value={entry.date}>{entry.date}</option>)}
                    </select>
                  </div>
                  <div>
              ...
              <Card>
                ...
                <div style={{ display: "grid", gap: 8 }}>
                  <div style={{ display: "grid", gridTemplateColumns: "80px 1fr 140px 110px", gap: 12, padding: "0 12px", fontSize: 12, fontWeight: 700, textTransform: "uppercase", color: "#64748b" }}>
                    <div>Use</div><div>Court</div><div>First game</div><div>Slots left</div>
                  </div>
          ...
          <Card>
            <div style={{ ...styles.headerRow, marginBottom: 16 }}>
              ...
              <div style={styles.row}>
                ...
                {result && !isPublicMode ? (
                  <button style={styles.button} onClick={() => exportCsv("filtered_schedule.csv", [["Division", "Date", "Time", "Court", "Home", "Away"], ...filteredSchedule.map((g) => [g.division, g.date, g.time, g.court, g.home, g.away])])}>
            ...
            {!result ? (
              <div style={{ border: "1px dashed #cbd5e1", borderRadius: 14, padding: 40, textAlign: "center", color: "#64748b" }}>{isPublicMode ? "Admin needs to publish a schedule before public visitors can browse it here." : "Generate a schedule to see schedule views here."}</div>
            ) : (
              <>
                <div style={{ display: "grid", gridTemplateColumns: "220px 280px 1fr", gap: 16, marginBottom: 16 }}>
                  ...
                  <div style={{ display: "flex", alignItems: "end", gap: 12, flexWrap: "wrap" }}>
                    ...
                    {scheduleTeamFilter !== "all" ? (
                      <div style={{ border: "1px solid #dbeafe", borderRadius: 12, background: "#eff6ff", padding: "12px 16px", fontSize: 13, color: "#1d4ed8" }}>Direct team view active for <strong>{scheduleTeamFilter}</strong></div>
                    ) : null}
                  </div>
                </div>
                    ...
                    <tbody>
                      {result.auditRows.map((row) => (
                        <tr key={row.team}>
                          ...
                          <td style={styles.td}><div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>{row.issues.length ? row.issues.map((issue) => <Badge key={issue} danger>{issue}</Badge>) : <Badge>OK</Badge>}</div></td>
                        </tr>
                      ))}
                    </tbody>
                  </table>
