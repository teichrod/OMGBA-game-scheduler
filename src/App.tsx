...
function scheduleFifthBoysDoubleheaderDay(teams, openSlots, schedule, unscheduled, config) {
  ...
  if (teamList.length % 2 === 1) {
    unscheduled.push({
      ...
      suggestion: "Use an even number of 5th Boys teams for an everyone-plays doubleheader day",
    });
    return;
  }
  ...
  if (slots.length < neededGames) {
    unscheduled.push({
      ...
      suggestion: "Enable more courts or times on that Saturday",
    });
    return;
  }
  ...
  if (selectedPairings.length < neededGames) {
    unscheduled.push({
      matchup: "5th Boys doubleheader day",
      reason: "Could not build two full rounds of pairings",
...
function generateScheduleEngine(config) {
  ...
  for (const division of DIVISIONS) {
    ...
    while (divisionTeams.some((t) => t.gamesScheduled < t.targetGames) && safety < 8000) {
      ...
      if (!scheduledOne) {
        unscheduled.push({
          matchup: needyTeams.map((t) => t.name).join(", "),
          reason: "No valid matchup or slot found during main pass",
          suggestion: "Add Saturdays, add courts on specific dates, or relax constraints",
        });
        break;
      }
    }
    ...
    while (team.gamesScheduled < team.targetGames && safety < 500) {
      ...
      if (!slot) {
        unscheduled.push({
          ...
          suggestion: "Add more Saturdays or enable more courts on active dates",
        });
        break;
      }
      const opponent = teams.find(
        (t) =>
          ...
          t.gamesScheduled < t.targetGames &&
          canPairInSlot(team, t, slot, config),
      );
      if (!opponent) {
        unscheduled.push({
          matchup: team.name,
          reason: "No opponent found",
          suggestion: "Allow more doubleheaders or create more slot capacity",
        });
        break;
      }
...
function runSelfChecks() {
  ...
  const checks = [
    ...
    },
    {
      ...
      pass: initial.dateCourtSettings[firstDate]?.every((c) => c.startTime === "8:00") ?? true,
    },
    {
      name: "Selected court date initializes safely",
      pass: initial.selectedDateForCourts === firstDate,
    },
    {
      name: "Total slots for date is non-negative",
      pass: getTotalSlotsForDate(initial, firstDate) >= 0,
    },
    {
      name: "Open slots are ordered early before late",
      pass: (() => {
        const slots = buildOpenSlots({
          ...initial,
          saturdays: [{ date: firstDate, enabled: true }],
        });
        return slots.length < 2 || getTimeIndex(slots[0].time) <= getTimeIndex(slots[1].time);
      })(),
    },
  ];
  return checks;
  ...
  return (
    <div style={styles.page}>
      <div style={styles.container}>
        ...
        {capacity.totalNeededGames > capacity.totalSlots ? (
        ...
        {activeTab === "schedule" ? (
          <Card>
            <div style={{ ...styles.headerRow, marginBottom: 16 }}>
              <SectionTitle icon={Trophy}>Schedule Views</SectionTitle>
              {result ? (
                <button
                  style={styles.button}
